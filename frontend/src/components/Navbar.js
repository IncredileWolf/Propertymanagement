...
export default function Navbar() {
  ...
  const handleSearch = (e) => {
    ...
    navigate("/search", {
      state: { gender: gender, atype: atype, furnish: furnish },
    });
  };
  ...
  const isadmin =
    state.loggedin.IsLoggedIn && sessionStorage.getItem("role") === "Admin"
      ...
      : false;
  const isowner =
    state.loggedin.IsLoggedIn && sessionStorage.getItem("role") === "Owner"
      ? true
      : false;
  const isuser =
    state.loggedin.IsLoggedIn && sessionStorage.getItem("role") === "Customer"
      ? true
      : false;
  const profile = isowner ? "/oprofile" : isuser ? "/uprofile" : "/";
  return (
    <div>
      <nav className="navbar fixed-top navbar-expand-lg navbar-dark  text-black fw-bold ">
